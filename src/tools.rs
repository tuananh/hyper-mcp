...
pub async fn tools_call(
    ...
    if let Some(plugin_name) = tool_cache.get(tool_name) {
        if let Some(plugin) = plugins.get_mut(plugin_name) {
            return match plugin.call::<&str, &str>("call", &json_string) {
                ...
                Err(e) => {
                    log::error!("Failed to execute plugin {}: {}, request: {:?}", plugin_name, e, request);
                    Err(
    ...
    Err(json!({"code": -32602, "message": format!("Tool '{}' not found in any plugin", tool_name)}).into_handler_error())
}
